...
namespace AudioCity.Controllers
{
    ...
    public class CreateNewGigController : Controller
    {
        ...
        public async Task<IActionResult> Index(GigViewModel GigViewModel)
        {
           ...
           /*     byte[] ThumbnailInByte = await FormFileHelper.GetBytes(GigViewModel.Thumbnail);
                string ThumbnailBase64 = Convert.ToBase64String(ThumbnailInByte);*/
                ...
                Gig NewGig = new Gig { CreatedBy = UserId, Title = GigViewModel.Title, Description = GigViewModel.Description, EstimatedDeliveryDays = GigViewModel.EstimatedDeliveryDays, Price = GigViewModel.Price, Category = GigViewModel.Category, Thumbnail = ThumbnailBlobReference, PortfolioFilePath = PortfolioBlobReference, PublishedOn = DateTime.Now };
        ...
        public IActionResult PreviewGig(Gig NewGig)
        {
            return View(NewGig);
        }
